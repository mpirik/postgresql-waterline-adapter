...
// region waterline-sequel fixes

/**
 * Overridden to not lower case primary or foreign keys
 * @param {string} tableColumnName
 * @param {*} value
 * @param {string} combinator
 * @param {Boolean} caseSensitive
 */
CriteriaProcessor.prototype.process = function process(tableColumnName, value, combinator, caseSensitive) {
  const tableName = this.getTableScope();
  ...
  const isKeyConstraint = currentSchema[tableColumnName].primaryKey || currentSchema[tableColumnName].foreignKey;

  const processMethod = _.isPlainObject(value) ? this.processObject : this.processSimple;

  processMethod.apply(this, [tableName, tableColumnName, value, combinator, isKeyConstraint || caseSensitive]);
};

/**
 * Override to fix a bug when columnName and tableName are used and an hydrated object is passed as query value.
 ...
 */
CriteriaProcessor.prototype.processObject = function processObject(tableName, tableColumnName, value, combinator, sensitive) {
  const modelSchema = this.schema[tableName];
  const currentSchema = this.schema[tableName].definition;
  const self = this;
  let tableColumnType;
  ...
  function expandCriteria(obj) {
    const child = self.findChild(tableColumnName);
    const sensitiveTypes = ['text', 'string'];
    let lower;

    if (_.isUndefined(obj[tableColumnName])) {
      for (const key of _.keys(obj)) {
        if (child && !self.isOperator(key)) {
          // NOTE: JG - The next two lines were modified to fix an issue when a hydrated query value is passed in.
          self.tableScope = null;
          self.expand(child, obj[key]);
          return;
        }

        // Recursively expand if value is an object
        if (_.isPlainObject(obj[key])) {
          return expandCriteria(obj[key]);
        }

        if (currentSchema[tableColumnName]) {
          tableColumnType = currentSchema[tableColumnName].type || currentSchema[tableColumnName];
        }

        lower = tableColumnType && sensitiveTypes.indexOf(tableColumnType) > -1;
        ...
        if (tableColumnType === 'date' && !_.isDate(obj[key])) {
        ...
        if (_.isArray(obj[key])) {
          // Handle arrays with null specifically
          const queryObj = [];
          let hasNull = false;
          let allValuesStrings = true;
          for (const value of obj[key]) {
            if (value === null) {
              hasNull = true;
            } else {
              allValuesStrings = allValuesStrings && _.isString(value);
              queryObj.push(value);
            }
          }
          ...
          if (queryObj.length) {
            self.queryString += `${self.buildParam(self.getTableAlias(), tableColumnName, !sensitive && allValuesStrings && lower)} `;
            self.prepareCriterion(key, queryObj);
            self.queryString += ' AND ';
          }
        } else {
          self.queryString += `${self.buildParam(self.getTableAlias(), tableColumnName, !sensitive && _.isString(obj[key]) && lower)} `;
          self.prepareCriterion(key, obj[key]);
          self.queryString += ' AND ';
        }
      }
    } else {
      // Handle the case where obj is a hydrated object of the column being queried
      let modelPropertyName = tableColumnName;
      for (const [schemaKey, definition] of Object.entries(modelSchema.attributes)) {
        if (definition.columnName === tableColumnName) {
          modelPropertyName = schemaKey;
          break;
        }
      }

      return self.expand(modelPropertyName, obj[tableColumnName]);
    }
  }
};
...
module.exports = {
  ...
  find(connectionName, modelName, queryObject, cb) {
    try {
      ...
      return db.query(query.query[0], query.values[0]).then((result) => {
        ...
        return result.rows;
      }).catch(this._handlePostgresError(cb, new Error(query.query[0])));
    } catch (ex) {
      if (cb) {
        return cb(ex);
      }
  ...
  create(connectionName, modelName, data, cb) {
    try {
      ...
      }).catch(this._handlePostgresError(cb, new Error(query.query)));
    } catch (ex) {
      if (cb) {
        return cb(ex);
      }
   ...
   * @param {function} [cb]
   * @param {string} error - Additional error details to append to thrown error
   * @returns {Function}
   * @private
   */
  _handlePostgresError(cb, error) {
    return function handlePostgresError(postgresError) {
      if (error.message) {
        postgresError.message += `  ${error.message}`;
      }

      postgresError.stack = (postgresError.stack || '') + error.stack;
      if (_.isFunction(cb)) {
